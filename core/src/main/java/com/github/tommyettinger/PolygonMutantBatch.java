...
/** A PolygonMutantBatch is used to draw 2D polygons that reference a texture (region). The class will batch the drawing commands
 * and optimize them for processing by the GPU.
 * <p>
 * To draw something with a PolygonMutantBatch one has to first call the {@link PolygonMutantBatch#begin()} method which will
 * setup appropriate render states. When you are done with drawing you have to call {@link PolygonMutantBatch#end()} which will
 * actually draw the things you specified.
 * <p>
 * All drawing commands of the PolygonMutantBatch operate in screen coordinates. The screen coordinate system has an x-axis
 ...
 * <p>
 * A PolygonMutantBatch is managed. In case the OpenGL context is lost all OpenGL resources a PolygonMutantBatch uses internally
 * get invalidated. A context is lost when a user switches to another application or receives an incoming call on Android. A
 * PolygonMutantBatch will be automatically reloaded after the OpenGL context is restored.
 * <p>
 * A PolygonMutantBatch is a pretty heavy object so you should only ever have one in your program.
 * <p>
 * A PolygonMutantBatch works with OpenGL ES 1.x and 2.0. In the case of a 2.0 context it will use its own custom shader to draw
 * all provided sprites. You can set your own custom shader via {@link #setShader(ShaderProgram)}.
 * <p>
 * A PolygonMutantBatch has to be disposed if it is no longer used.
 * @author mzechner
 * @author Stefan Bachmann
 * @author Nathan Sweet 
 * @author Tommy Ettinger */
public class PolygonMutantBatch implements PolygonBatch {
	...
	public float colorPacked = Color.WHITE_FLOAT_BITS; // THIS IS PUBLIC BECAUSE PACKAGE-PRIVATE GOT US INTO THIS MESS!
	...
	/** Constructs a PolygonMutantBatch with the default shader, 2000 vertices, and 4000 triangles.
	 * @see #PolygonMutantBatch(int, int, ShaderProgram) */
	public PolygonMutantBatch() {
		this(2000, null);
	}

	/** Constructs a PolygonMutantBatch with the default shader, size vertices, and size * 2 triangles.
	 * @param size The max number of vertices and number of triangles in a single batch. Max of 32767.
	 * @see #PolygonMutantBatch(int, int, ShaderProgram) */
	public PolygonMutantBatch(int size) {
		this(size, size * 2, null);
	}

	/** Constructs a PolygonMutantBatch with the specified shader, size vertices and size * 2 triangles.
	 * @param size The max number of vertices and number of triangles in a single batch. Max of 32767.
	 * @see #PolygonMutantBatch(int, int, ShaderProgram) */
	public PolygonMutantBatch(int size, ShaderProgram defaultShader) {
		this(size, size * 2, defaultShader);
	}

	/** Constructs a new PolygonMutantBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards,
	 ...
	 * the ones expect for shaders set with {@link #setShader(ShaderProgram)}. See {@link MutantBatch#createDefaultShader()}.
	 ...
	 * @param defaultShader The default shader to use. This is not owned by the PolygonMutantBatch and must be disposed separately.
	 *           May be null to use the default shader. */
	public PolygonMutantBatch(int maxVertices, int maxTriangles, ShaderProgram defaultShader) {
		...
		if (defaultShader == null) {
			shader = MutantBatch.createDefaultShader();
			ownsShader = true;
		} else
	...
	@Override
	public void begin () {
		if (drawing) throw new IllegalStateException("PolygonMutantBatch.end must be called before begin.");
		renderCalls = 0;
	...
	@Override
	public void end () {
		if (!drawing) throw new IllegalStateException("PolygonMutantBatch.begin must be called before end.");
	...
	public void setColor (float r, float g, float b, float a) {
		colorPacked = NumberUtils.intBitsToFloat(((int)(255 * a) << 24 & 0xFE000000)
	...
	public void setRGBAColor (int r, int g, int b, int a) {
		colorPacked = NumberUtils.intBitsToFloat((a << 24 & 0xFE000000)
	...
	public Color getColor () {
		final int intBits = NumberUtils.floatToIntBits(colorPacked);
	...
	public void draw (PolygonRegion region, float x, float y) {
		if (!drawing) throw new IllegalStateException("PolygonMutantBatch.begin must be called before draw.");
