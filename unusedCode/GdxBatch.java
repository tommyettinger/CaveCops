...
public class GdxBatch implements Batch {
	...
	/** Constructs a GdxBatch with one buffer and the default shader.
	 * @see GdxBatch#GdxBatch(int, ShaderProgram) */
	public GdxBatch(int size) {
		this(size, null);
	}
	...
	public void setRGBAColor (final int color) {
		...
		this.colorPacked = NumberTools.reversedIntBitsToFloat(color & -2);
	}

	public void setRGBAColor (int r, int g, int b, int a) {
		colorPacked = NumberTools.intBitsToFloat((a << 24 & 0xFE000000) | (b << 16 & 0xFF0000) | (g << 8 & 0xFF00) | (r & 0xFF));
